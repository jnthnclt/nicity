...
public class GroupAreas extends RectPaintedArea implements IPaintableCollection {
    ...
    public void add(ItemArea... _areas) {
        ...
        for(int i=0;i<items.length;i++) {
            if (!(items[i].x instanceof GroupV)) {
                items[i].x = new GroupV(items[i].x,0);
            }
            if (!(items[i].y instanceof GroupV)) {
                items[i].y = new GroupV(items[i].y,1);
            }
            if (!(items[i].w instanceof GroupV)) {
                items[i].w = new GroupV(items[i].w,2);
            }
            if (!(items[i].h instanceof GroupV)) {
                items[i].h = new GroupV(items[i].h,3);
            }          
        }
    }
    ...
    class GroupV extends V_D {
        ...
        public double getV() {
            if (mode == 0) {
                return MinMaxDouble.unzeroToOne(x.getV(), x.getV()+w.getV(), was.getV());
            }
            else if (mode == 1) {
                return MinMaxDouble.unzeroToOne(y.getV(), y.getV()+h.getV(), was.getV());
            }
            else if (mode == 2) {
                return w.getV()*was.getV();
            }
            else {
                return h.getV()*was.getV();
            }
        ...
        public void setV(double _v) {
            if (mode == 0) {
                was.setV(MinMaxDouble.unzeroToOne(x.getV(), x.getV()+w.getV(),_v));
            }
            else if (mode == 1) {
                was.setV(MinMaxDouble.unzeroToOne(y.getV(), y.getV()+h.getV(),_v));
            }
            else if (mode == 2) {
                was.setV(w.getV()/_v);
            }
