...
public class JsonApacheHttpClient {
    ...
    public JsonApacheHttpClient(ObjectMapper mapper, org.apache.commons.httpclient.HttpClient client,
    ...
    public <J> J get(String path, Class<J> jsonClass) {
        GetMethod get = new GetMethod(path);
        try {
            return execute(get, jsonClass);
        } catch (Exception e) {
    ...
    public <J> J postJson(String path, String postJsonBody, Class<J> jsonClass) {
        try {
    ...
    <J> J execute(HttpMethod method, Class<J> jsonClass) throws IOException {
        ...
        if (statusLine.getStatusCode() >= 200 && statusLine.getStatusCode() < 300) {
            return mapper.readValue(responseBody, jsonClass);
        } else {
            throw new RuntimeException("Status code:" + statusLine.getStatusCode() + " reason:" + statusLine.getReasonPhrase() + " response:" + new String(responseBody));
        }
    }
